<template>
  <div class="page-container">
    <div class="page-title">车辆品牌管理</div>
    ...
    <el-table :data="baseDataStore.vehicleBrands" stripe border>
      ...
      <el-table-column label="操作" width="180" align="center" fixed="right">
        <template #default="{ row }">
          ...
          <el-button type="danger" link size="small" @click="deleteBrand(row.id)" v-if="row.status !== 'active'">删除</el-button>
          <el-button type="warning" link size="small" @click="toggleStatus(row)" v-else>禁用</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog v-model="dialogVisible" :title="editingId ? '编辑品牌' : '新增品牌'" width="600px" destroy-on-close>
      <el-form :model="form" :rules="rules" ref="formRef" label-width="100px">
        <el-form-item label="品牌名称" prop="name">
          <el-input v-model="form.name" placeholder="如：宝马" />
        </el-form-item>
        <el-form-item label="英文名称" prop="nameEn">
          <el-input v-model="form.nameEn" placeholder="如：BMW" />
        </el-form-item>
...
function submit() {
  formRef.value?.validate((valid) => {
    ...
    if (editingId.value) {
      baseDataStore.updateVehicleBrand(editingId.value, form.value)
      ElMessage.success('更新成功')
    } else {
      baseDataStore.addVehicleBrand(form.value)
...
function deleteBrand(id) {
  ElMessageBox.confirm('确定删除该品牌吗？', '提示', { type: 'warning' }).then(() => {
    baseDataStore.deleteVehicleBrand(id)
...
function toggleStatus(row) {
  ...
  baseDataStore.updateVehicleBrand(row.id, { status: newStatus })
